...
impl Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        let index = self
            .chunks
            .iter()
            .position(|chunk| chunk.chunk_type().bytes() == bytes);

        match index {
            Some(i) => {
                let removed_element = self.chunks.remove(i);
                Ok(removed_element)
            }
            None => Err("No element matches the condition")?,
        }
    }
    fn header(&self) -> &[u8; 8] {
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        ...
        self.chunks
            .iter()
            .find(|chunk| chunk.chunk_type().bytes() == bytes)
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        let mut offset = 0;
        if value[offset..Self::STANDARD_HEADER.len()] != Self::STANDARD_HEADER {
        ...
        loop {
            let length = u32::from_be_bytes(value[offset..offset + 4].try_into()?) as usize;
            let read_until = offset + length + 4 + 4 + 4;
            let chunk_bytes = value[offset..read_until].to_vec();
...
impl Display for Png {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let png_string = String::from_iter(
            self.chunks
                .iter()
                .map(|c| c.data_as_string().unwrap_or_default()),
        );
        write!(f, "{}", png_string)
    }
}
